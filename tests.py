...
class TestMethods(unittest.TestCase):
    ...
    def test_geo_1(self):
        gen_geo_inc('csv_data/11/общая численность населения,о.csv',
                    'csv_data/analysis/численность, ряды динамики.csv')

    def test__(self):
        df_to_geojson('csv_data/analysis/численность, ряды динамики.csv')

    def test_add_geo(self):
        add_data('geojson_data/здоровьеАрхангельска.geojson',
                 'csv_data/analysis/численность, ряды динамики.csv',
                 'прирост','абсолют. прирост цепных&темп прироста цепных, %',
                 'abs&pace in percent','geojson_data/pri.geojson')

    def test_read_j(self):
        read_geojson('geojson_data/pri.geojson')
