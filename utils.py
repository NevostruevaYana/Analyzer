...
SPACE = ' '
DATABASE_DIR = 'csv_data/database/'
YEARS_CSV = DATABASE_DIR + 'годы.csv'
DISTRICT_CSV = DATABASE_DIR + 'районы.csv'
CSV = '.csv'
EXCEL = '.xlsx'
NONE_STR = 'None'
DATA_DIR = 'csv_data/'
...
RES = 'results.csv'
DISTRICT_POLYGON = 'geojson_data/districts/'
...
CSV_PROP = DATABASE_DIR + 'new_properties.csv'
CSV_PROP_IND = ['смертность на 100 тыс. нас.',                  # 0
                ...
                'заболеваемость от 18 лет на 100 тыс. нас.'     # 6
                'младенческая смертность на 1000 нас.',         # 7
                'удельный вес сельского нас., %',               # 8
                'работающее население, %',                      # 9
                'число случаев временной нетрудоспособности',   # 10
                'безработица, %',                               # 11
                'психические расстройства в возр. от 18 лет, %',# 12
                'невротические и соматоформные расстройства в возр. от 18 лет, %',  # 13
                'другие непсихотические расстройства в возр. от 18 лет, %',         # 14
                'алкоголизм в возр. от 18 лет, %','наркомания от 18 лет, %',        # 15
                'число лиц, которым оказана медицинская помощь при выездах на 100 тыс. нас.',   # 16
                'численность работающих мужчин',                # 17
                'работающих мужчин, %',                         # 18
                'работающих женщин, %'                          # 19
                ]
CSV_11 = DATABASE_DIR + '11_абсолютное значение.csv'
...
CSV_31 = DATABASE_DIR + '31_значение.csv'
...
CSV_23_0_14 = DATABASE_DIR + '23_от 0-14 лет абс..csv'
CSV_23_15_17 = DATABASE_DIR + '23_15-17 лет абс..csv'
CSV_23_18_ = DATABASE_DIR + '23_от 18 абс..csv'
...
CSV_27_0_14 = DATABASE_DIR + '27_0-14 лет абс..csv'
CSV_27_15_17 = DATABASE_DIR + '27_15-17 лет абс..csv'
CSV_27_18_60 = DATABASE_DIR + '27_18-60 абс..csv'
...
CSV_28_B = DATABASE_DIR + '28_мужчины знач.csv'
CSV_28_G = DATABASE_DIR + '28_женщины знач..csv'
...
def append_cols_to_file(file_name, df):
    if os.path.exists(file_name):
        read_df = pd.read_csv(file_name)
        del df[0:3]
        read_df = pd.concat([read_df, df], axis=1)
        read_df.to_csv(file_name, index=False)
    else:
        df.to_csv(file_name, index=False)
